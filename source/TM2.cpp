...
public:
    ...
    //"A configuration of a Turing machine is an ordered tripple (x, q, k) where x denotes the string on the tape, q denotes the machine's current state, and k denotes the position of the machine on the tape."
    //I will be piping these outputs to PDA.cpp which will be running TM2.cpp and capturing the outputs as the "configuration"
    void printTapeContent();
    void printState();
    void printTapeHeadPosition();
...
bool turingMachine::runTM()
{
	...
	//int whichState = 0;
	//vector<unordered_map<char, tuple<char, int, int> > > delta;
void turingMachine::printTapeContent() {
    cout << inputTape[readHead] << endl;
}
void turingMachine::printState() {
    cout << whichState << endl;
}
void turingMachine::printTapeHeadPosition() {
    cout << readHead << endl;
}
